...
public class ChangeWorkerWindow extends Stage {
    ...
    Label titleLabel = new Label("Зміна запису \"Працівник\"");
    ...
    public ChangeWorkerWindow(Stage mainStage, WorkerDTO mainWorker,
                              List<WorkerDTO> workers, Tab primaryTab) {

        firstNameField.setText(mainWorker.getFirstName());
        lastNameField.setText(mainWorker.getLastName());
        phoneField.setText(mainWorker.getPhoneNumber());
        birthdayField.setValue(mainWorker.getBirthdayDate());
        stageField.getValueFactory().setValue(mainWorker.getStage());
        ...
        listOfShift.forEach(e -> {
            ...
            if (e.getId() == mainWorker.getShiftId()) {
        ...
        listOfUnit.forEach(e -> {
            ...
            if (mainWorker.getUnitIds().contains(e.getId())) {
        ...
        listOfPositions.forEach(e -> {
            ...
            if (mainWorker.getPositionIds().contains(e.getId())) {
                checkBox.setSelected(true);
            }
        });
...
//        root.setGridLinesVisible(true);
        ...
        button.setOnAction(e -> changeWorker(mainWorker, mainStage, workers, primaryTab));

    }

    private void changeWorker(WorkerDTO mainWorker, Stage mainStage,
                              List<WorkerDTO> workers, Tab primaryTab) {
        WorkerDTO workerDTO = new WorkerDTO();
        workerDTO.setId(mainWorker.getId());
        ...
        if (message.equals("")) {
            ...
            workers = workers.stream().map(e -> {
                WorkerDTO worker = e;
                if (e.getId() == mainWorker.getId()) {
                    worker = workerDTO;
                }
                return worker;
            }).collect(Collectors.toList());
//            mainStage.setScene(new Scene(InformationalTable.getWorkerTable(mainStage, workers)));
