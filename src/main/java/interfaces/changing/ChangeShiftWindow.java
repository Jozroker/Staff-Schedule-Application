...
public class ChangeShiftWindow extends Stage {

    ShiftService shiftService = ShiftServiceImpl.getInstance();

    Label titleLabel = new Label("Зміна запису \"Зміна\"");
    ...
    public ChangeShiftWindow(Stage mainStage, ShiftDTO mainShift,
                             Tab primaryTab) {

        nameField.setText(mainShift.getName());
        startTimeField.getValueFactory().setValue(mainShift.getBeginTime());
        endTimeField.getValueFactory().setValue(mainShift.getEndTime());
...
//        root.setGridLinesVisible(true);
        ...
        button.setOnAction(e -> changeShift(mainShift, mainStage, primaryTab));
    }

    private void changeShift(ShiftDTO mainShift, Stage mainStage, Tab primaryTab) {
...
//            mainStage.setScene(new Scene(InformationalTable.getShiftTable(mainStage)));
